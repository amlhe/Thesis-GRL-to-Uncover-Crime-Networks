...
image(1:nrow(WR), 1:ncol(WR), WR, 
      col = colorRampPalette(brewer.pal(9, "Greys")[c(1, 8)])(30),
      axes = FALSE, xlab = "", ylab = "", asp = 1, xlim = c(0, nrow(WR)+1), ylim = c(0,ncol(WR)+1))
rect(0, 0, ncol(WR)+1, nrow(WR)+1, border = "black", lwd = 2)
fun_grob <- function(){
  grid.echo()
  grid.grab()
}
...
G_WR_mis <- graph.adjacency(WR_mis, mode="undirected", diag=FALSE)
...
Z_DM_mis <- esbm(WR_mis, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, beta_DM = 12/50, H_DM = 50)

# DIRICHLET PROCESS (CRP)
# ------------------------------------
my_prior <- "DP"
Z_DP <- esbm(Y, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, alpha_PY = 8, sigma_PY = 0)
Z_DP_mis <- esbm(WR_mis, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, alpha_PY = 8, sigma_PY = 0)

# PITMAN-YOR PROCESS
# ------------------------------------
my_prior <- "PY"
Z_PY <- esbm(Y, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, alpha_PY = -0.350, sigma_PY = 0.725)
Z_PY_mis <- esbm(WR_mis, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, alpha_PY = -0.350, sigma_PY = 0.725)
...
# Save the output
save(Z_DP,Z_PY,Z_GN,Z_DM,file="Application/Posterior_No_Attributes_WR_fixed.RData")
load("Application/Posterior_No_Attributes_WR_fixed.RData")

save(Z_DP_mis,Z_PY_mis,Z_GN_mis,Z_DM_mis,file="Application/Posterior_No_Attributes_WR_mis.RData")
load("Application/Posterior_No_Attributes_WR_mis.RData")
...
for (t in 1:dim(Z_DP_WAIC_mis)[2]){
  LL[,t]<-sampleLL(Z_DP_WAIC_mis[,t],WR_mis,a,b)
  if (t%%1000 == 0){print(paste("Iteration:", t))}
}
...
for (t in 1:dim(Z_DM_WAIC_mis)[2]){
  LL[,t]<-sampleLL(Z_DM_WAIC_mis[,t],WR_mis,a,b)
  if (t%%1000 == 0){print(paste("Iteration:", t))}
}
...
for (t in 1:dim(Z_PY_WAIC_mis)[2]){
  LL[,t]<-sampleLL(Z_PY_WAIC_mis[,t],WR_mis,a,b)
  if (t%%1000 == 0){print(paste("Iteration:", t))}
}
...
for (t in 1:dim(Z_GN_WAIC_mis)[2]){
  LL[,t]<-sampleLL(Z_GN_WAIC_mis[,t],WR_mis,a,b)
  if (t%%1000 == 0){print(paste("Iteration:", t))}
}
...
plot(G_WR, vertex.size = 10, vertex.color = node_colors, vertex.label = NA, main = paste("Number of communities:", length(unique(memb_Z_DP))))
...
plot(G_WR, vertex.size = 10, vertex.color = node_colors, vertex.label = NA, main = paste("Number of communities:", length(unique(memb_Z_PY))))
...
plot(G_WR, vertex.size = 10, vertex.color = node_colors, vertex.label = NA, main = paste("Number of communities:", length(unique(memb_Z_DM))))
...
plot(G_WR, vertex.size = 10, vertex.color = node_colors, vertex.label = NA, main = paste("Number of communities:", length(unique(memb_Z_GN))))
