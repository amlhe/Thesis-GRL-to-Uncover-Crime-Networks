...
sparse_i_SN <- read.table("SN edgelist/sparse_i.txt", header = FALSE)$V1
sparse_j_SN <- read.table("SN edgelist/sparse_j.txt", header = FALSE)$V1
num_nodes <- 146
SN <- matrix(0, nrow = num_nodes, ncol = num_nodes)
for (i in 1:length(sparse_i_SN)) {
  SN[sparse_i_SN[i], sparse_j_SN[i]] <- 1
  SN[sparse_j_SN[i], sparse_i_SN[i]] <- 1
}
...
image(1:nrow(SN), 1:ncol(SN), SN, 
      col = colorRampPalette(brewer.pal(9, "Greys")[c(1, 8)])(30),
      axes = FALSE, xlab = "", ylab = "", asp = 1, xlim = c(0, nrow(SN)+1), ylim = c(0,ncol(SN)+1))
rect(0, 0, ncol(SN)+1, nrow(SN)+1, border = "black", lwd = 2)
fun_grob <- function(){
  grid.echo()
  grid.grab()
}
...
sparse_i_SN <- read.table("Missing links/SN/sparse_i.txt", header = FALSE)$V1
sparse_j_SN <- read.table("Missing links/SN/sparse_j.txt", header = FALSE)$V1
num_nodes <- 146
SN_mis <- matrix(0, nrow = num_nodes, ncol = num_nodes)
for (i in 1:length(sparse_i_SN)) {
  SN_mis[sparse_i_SN[i], sparse_j_SN[i]] <- 1
  SN_mis[sparse_j_SN[i], sparse_i_SN[i]] <- 1
}
...
G_SN_mis <- graph.adjacency(SN_mis, mode="undirected", diag=FALSE)
...
Z_DM_mis <- esbm(SN_mis, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, beta_DM = 12/50, H_DM = 50)

# DIRICHLET PROCESS (CRP)
# ------------------------------------
my_prior <- "DP"
Z_DP <- esbm(Y, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, alpha_PY = 8, sigma_PY = 0)
Z_DP_mis <- esbm(SN_mis, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, alpha_PY = 8, sigma_PY = 0)

# PITMAN-YOR PROCESS
# ------------------------------------
my_prior <- "PY"
Z_PY <- esbm(Y, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, alpha_PY = -0.350, sigma_PY = 0.725)
Z_PY_mis <- esbm(SN_mis, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, alpha_PY = -0.350, sigma_PY = 0.725)

# GNEDIN PROCESS
# ------------------------------------
my_prior  <- "GN"
Z_GN <- esbm(Y, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, gamma_GN = 0.3)
Z_GN_mis <- esbm(SN_mis, my_seed, N_iter, my_prior, my_z, a = 1, b = 1, gamma_GN = 0.3)


# Save the output
save(Z_DP,Z_PY,Z_GN,Z_DM,file="Application/Posterior_No_Attributes_SN_fixed.RData")
load("Application/Posterior_No_Attributes_SN_fixed.RData")

save(Z_DP_mis,Z_PY_mis,Z_GN_mis,Z_DM_mis,file="Application/Posterior_No_Attributes_SN_mis.RData")
load("Application/Posterior_No_Attributes_SN_mis.RData")
...
WAIC(LL)$WAIC
# Traceplot
plot(ts(LL[index_traceplot,]),xlab="",ylab="",ylim=c(-0.5,0.1), main = "Dirichlet process")
...
WAIC(LL)$WAIC
# Traceplot
plot(ts(LL[index_traceplot,]),xlab="",ylab="",ylim=c(-0.5,0.1), main = "Dirichlet multinomial")
...
WAIC(LL)$WAIC
# Traceplot
plot(ts(LL[index_traceplot,]),xlab="",ylab="",ylim=c(-0.5,0.1), main = "Pitman-Yor process")
...
WAIC(LL)$WAIC
# Traceplot
plot(ts(LL[index_traceplot,]),xlab="",ylab="",ylim=c(-0.5,0.1), main = "Gnedin process")
...
write.csv(memb_Z_DP_mis, "../ESBM ROC/memb_Z_DP_mis.csv", row.names = FALSE)
write.csv(memb_Z_DM_mis, "../ESBM ROC/memb_Z_DM_mis.csv", row.names = FALSE)
write.csv(memb_Z_PY_mis, "../ESBM ROC/memb_Z_PY_mis.csv", row.names = FALSE)
write.csv(memb_Z_GN_mis, "../ESBM ROC/memb_Z_GN_mis.csv", row.names = FALSE)
write.csv(Y, "../ESBM ROC/Y.csv", row.names = FALSE)
...
for (t in 1:dim(Z_DP_WAIC_mis)[2]){
  LL[,t]<-sampleLL(Z_DP_WAIC_mis[,t],SN_mis,a,b)
  if (t%%1000 == 0){print(paste("Iteration:", t))}
}
...
for (t in 1:dim(Z_DM_WAIC_mis)[2]){
  LL[,t]<-sampleLL(Z_DM_WAIC_mis[,t],SN_mis,a,b)
  if (t%%1000 == 0){print(paste("Iteration:", t))}
}
...
for (t in 1:dim(Z_PY_WAIC_mis)[2]){
  LL[,t]<-sampleLL(Z_PY_WAIC_mis[,t],SN_mis,a,b)
  if (t%%1000 == 0){print(paste("Iteration:", t))}
}
...
for (t in 1:dim(Z_GN_WAIC_mis)[2]){
  LL[,t]<-sampleLL(Z_GN_WAIC_mis[,t],SN_mis,a,b)
  if (t%%1000 == 0){print(paste("Iteration:", t))}
}
...
plot(G_SN, vertex.size = 10, vertex.color = node_colors, vertex.label = NA, main = paste("Number of communities:", length(unique(memb_Z_DP))))
...
plot(G_SN, vertex.size = 10, vertex.color = node_colors, vertex.label = NA, main = paste("Number of communities:", length(unique(memb_Z_PY))))
...
plot(G_SN, vertex.size = 10, vertex.color = node_colors, vertex.label = NA, main = paste("Number of communities:", length(unique(memb_Z_DM))))
...
plot(G_SN, vertex.size = 10, vertex.color = node_colors, vertex.label = NA, main = paste("Number of communities:", length(unique(memb_Z_GN))))
